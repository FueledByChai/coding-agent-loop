...
loop_checks() {
  ...
  for s in loop-config backlog-status open-ticket-pr release-notes loop-kit-sync proof-gate coverage-ratchet review-status decisions prompt-check sprint check-list ruleset-check pr-readiness with-test-postgres compose-smoke merge-queue; do
