...
Still to supply:
  1. scripts/check.sh: the definition of done for this project (exit non-zero on anything not
     ...
     scripts/check-list.sh --self-test, scripts/loop-kit-sync.sh --check,
...
self_test() {
  ...
  for f in loop-config backlog-status open-ticket-pr release-notes loop-kit-sync proof-gate coverage-ratchet review-status decisions prompt-check sprint loop-tui check-list; do
