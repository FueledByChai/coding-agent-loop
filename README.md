...
| `scripts/loop-tui.sh` | the loop's state as one screen: the dashboard (the sprint, the next ticket and the command that claims it, what is left, the stories), and the `stories`, `open`, and `show <id>` views; `--width` fixes the width, `--render` prints one frame and exits, and on a terminal it is interactive — `j`/`k` move the marker, `<sp>` opens the marked item in full, `a` adds a ticket to the sprint and `x` takes one out (both through `scripts/sprint.sh`, never committed), `s` and `o` switch views, `r` fetches from origin and redraws, `q` quits; `--keys '<keys>'` drives those keys and prints the frame they left, so the whole program runs without a terminal |
