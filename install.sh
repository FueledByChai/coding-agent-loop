...
self_test() {
  ...
  for f in loop-config backlog-status open-ticket-pr release-notes loop-kit-sync proof-gate coverage-ratchet review-status decisions prompt-check sprint check-list ruleset-check pr-readiness reference-check with-test-postgres compose-smoke merge-queue review-workers worker-domain; do
    [ -x "$dir/scripts/$f.sh" ] || { echo "self-test: scripts/$f.sh missing or not executable"; exit 1; }
  done
  for f in merge-queue.py merge-queue-tests.py review-workers.py review-workers-tests.py worker-domain.py worker-domain-tests.py; do
  ...
  (cd "$dir" && scripts/worker-domain.sh --self-test | grep -q 'worker-domain self-test passed') \
    || { echo "self-test: installed worker domain self-test failed"; exit 1; }
