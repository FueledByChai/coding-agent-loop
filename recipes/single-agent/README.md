...
2. Copy `AGENTS.template.md` to `AGENTS.md` and replace the bracketed project rules.
3. Provide an application-owned `scripts/check.sh`. It must run non-interactively and return
   nonzero on failure. Keep targeted local test commands in the project rules.
4. Copy `ci.yml` to `.github/workflows/ci.yml` and add the project's toolchain setup before the
   check step.
5. Apply `ruleset.json` to the default branch and enable repository auto-merge and automatic
   deletion of merged branches.
6. Validate the copied recipe with `./recipes/single-agent/validate.sh` before adapting it.
...
7. Open the cutover pull request, run the one required check, and merge it. Close or supersede
...
5. Reviews the final diff once, commits, pushes, and opens a ready pull request.
6. Enables rebase auto-merge and lets the required full CI check run once.
7. Verifies the merge, closes the Bead, publishes Beads state, and only then selects more work.

If CI fails, fix the actual failure and push once. If feedback contains several concrete defects,
batch the accepted fixes into one push. Do not start an unbounded review/re-review cycle.
